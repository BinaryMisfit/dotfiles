...
| `dot_scripts/chezmoi-self-heal-check.{ps1,sh}` | `~/.scripts/chezmoi-self-heal-check.{ps1,sh}` | Detection-only report on chezmoi drift, missing `run_once` side effects, and interrupted-apply state — never fixes anything automatically (TODO-5). Manually invoked, not scheduled yet |
| `run_once_configure-chezmoi-apply-hooks.{ps1,sh}.tmpl` | `~/.config/chezmoi/chezmoi.{yaml,toml}` (additive append) | One-time setup of chezmoi's native `hooks.apply.pre`/`post`, the mechanism the interrupted-apply tier above actually depends on — see [ADR 0023](adr/0023-scripted-toml-yaml-writes-are-additive-only.md) |
| `dot_scripts/chezmoi-apply-marker.ps1` / `executable_chezmoi-apply-marker.sh` | `~/.scripts/chezmoi-apply-marker.{ps1,sh}` | Sets/clears the `~/.chezmoi-apply-in-progress` sentinel the hooks above invoke |
...
| `output-styles/{hailey,alexia,aphrodite,callie,daisy}.md` | `~/.claude/output-styles/` | Home | Persona output-styles. `daisy.md` added 2026-09-07 — fifth persona; onboarding day started same day, still filling in as she writes her own voice |
