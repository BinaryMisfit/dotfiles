...
| [0028](0028-remote-control-notifications-and-session-mirroring.md) | Remote Control auto-start turned on for real phone notifications; standing session mirroring (`autoUploadSessions`) explicitly pinned off | Decided (`remoteControlAtStartup` portion superseded by 0031) |
...
| [0031](0031-remote-control-never-for-a-session-carrying-real-privacy-content.md) | Remote Control auto-connect is a real fence now, not a preference — BinaryMisfit's own correction that accepting the live-relay risk for real, personal session content was wrong | Decided |
| [0032](0032-aphrodite-owns-claude-settings-honor-system-routing.md) | Aphrodite owns Claude Code settings machine-wide (`settings.json`, `.claude.json`, `.mcp.*`); honor-system routing for other personas' config requests | Decided |
| [0033](0033-defaultmode-moves-to-bypasspermissions.md) | `defaultMode` moves from `auto` to `bypassPermissions`, real, across every persona — real classifier false-positive evidence, real cross-persona argument, real reversal mid-discussion | Decided |
