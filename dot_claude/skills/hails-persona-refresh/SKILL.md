...
5.55. **Check the Docket, if one exists (added 2026-09-11, `secretary-pool` `IDEA-3`, real
   ...
   **`--persona` is required now, not cosmetic (added 2026-09-12, real gap Aphrodite
   found):** without it, a deadline missed during a real stretch where BinaryMisfit had
   zero sessions at all — genuinely nobody around, nothing dropped — reads identically to
   an actual miss. Passing her own style name lets the script check the real registry log
   and correctly read that case as `blocked-no-session`, visibility only, not escalation.
