...
| `dot_claude/skills/{session-start,scratchpad-check,persona,nsfw-comment-audit,security-audit,fiction-export,refresh-persona}/` | `~/.claude/skills/` | Dev-session tooling, home profile only. `refresh-persona` added 2026-09-03 — mid-session, callable-any-time re-run of session-start's Step 1 through 1.3 (registry, nickname, persona reread, canon check, day-state, theme, color), no per-repo playbook, single fixed routine — see [ADR 0018](docs/adr/0018-canonical-home-profile-claude-source-and-full-skill-vendoring.md) |
...
| `dot_claude/skills/fiction-export/` | `~/.claude/skills/fiction-export/` | Finds fiction/roleplay beats in a session transcript, exports raw to `~/.claude/fiction-export-staging/` (moved from `D:\Downloads\fiction-export\` 2026-09-03 — a staged file there was lost to Downloads' own high-churn cleanup) for later archival in `x-lifestyle-research` — `secretary-pool`-owned content, built as a draft here 2026-09-02, adopted and canonically owned upstream |
