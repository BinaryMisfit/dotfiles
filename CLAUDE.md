...
| `dot_claude/settings.json.tmpl` | `~/.claude/settings.json` | Claude Code permissions. `autoMode.environment` (home-profile only) declares BinaryMisfit's homelab as a trusted Auto Mode environment — see [ADR 0025](docs/adr/0025-auto-mode-environment-trust-block-homelab-scoped.md). `remoteControlAtStartup: false` (all four personas — reversed back to off 2026-09-09, BinaryMisfit's own real-time ask, reasoning pending — see that ADR's own addendum) and `autoUploadSessions: false` (no standing session mirror to claude.ai) — see [ADR 0028](docs/adr/0028-remote-control-notifications-and-session-mirroring.md). `advisorModel: "sonnet"` (was `"opus"`, changed 2026-09-06 — the `advisor` tool stays useful at the cheaper model for this workflow). Also carries `timeFormat: "24-hour"`, `promptSuggestionEnabled: false`, `dialogExpiry: "never"`, `autoContinueAtUsageLimit: false`, `crossSessionInbound: "accept"` — plain CLI preference toggles, added 2026-09-06, verified via a live file-change monitor against a known-good snapshot before landing here, no ADR needed. `awaySummaryEnabled: false` added the same way, 2026-09-07. `theme: "auto"` (was `"dark"`)
set live 2026-09-08 via `/theme`, real reason: lets the Windows Terminal pane-coloring
mechanism (see the terminal-settings and `claude-launch.ps1` rows below) actually show
through Claude Code's own UI instead of a fixed dark palette fighting it — confirmed
directly against the live setting before updating the template, not assumed. `/color`'s own session color does **not** persist to this file or anywhere on disk — confirmed directly, session-only by design, nothing to template |
